...
def call_postgres(conn ,date_interval, time_interval):
    ...
    # 路段，上下游节点，进出口道信息
    # 筛选关联报警数据
    sql1 = "select * from " + CONTENT.alarm_match_data + " where  rdsectid != 'UTRSS001116' and date_day ='{0}' " \
           "and time_interval ='{1}' and " \
           "((length<=300  and (up_to_down_match_num>= 1 OR down_to_up_match_num>= 1)) or(length<500 and " \
           "length>300 and ((up_to_down_match_num)>=(round(length/100,2)-2)" \
           " OR down_to_up_match_num>=(round(length/100,2)-2) )) or" \
           "(length<700 and length>500 and (up_to_down_match_num>=(round(20*length/1000,2)-7)" \
           "OR down_to_up_match_num>=(round(20*length/1000,2)-7) ))OR(length>=700 " \
           "and (up_to_down_match_num>=(round(40*length/1000,2)-21) OR down_to_up_match_num>=" \
           "(round(40*length/1000,2)-21)))) order by rdsectid"\
    ...
    for i in group_result:
        ...
        for j in node_list:
            ...
            if up_node_num + down_node_num > 2:
                ...
                pass
            elif up_node_num + down_node_num == 2 and up_node_num-down_node_num != 0:
                key_node_list.append(j)
                key_subset = {'KeyNode': j, 'all_node': distinct_node, 'Subset': np.array(match_data['rdsectid']).tolist()}
                subset.append(key_subset)
            else:
...
def max_alarm_node(group_data):
    # print(group_data)
    group_group_data = group_data.groupby(['group_id'])     # 分组
    # 求每组报警次数最大值
...
def insert_postgres(group_result, date_interval, time_interval):
    ...
    # group_data = call_group_data(date_interval, time_interval)
    # conflict_group = []
    # print(rdsect_set)
    ...
    for i in rdsect_set:
        ...
        # 子团划分
        # if len(subset) > 0:
        #     conflict_group.append(group_id)
        #
        #     for j in range(len(subset)):
        #         key_node_id = subset[j]['KeyNode']
        #         sub_group_id = 'subset_' + str(key_node_id)
        #         sub_rdsectid = subset[j]['Subset']
        #
        #         sub_node = subset[j]['all_node']
        #         sub_group = {'SubGroupId': sub_group_id, 'SubRdsectId': sub_rdsectid, 'SubNode': sub_node}
        #         for m in range(len(sub_rdsectid)):
        #             chose_rdsectid.append(sub_rdsectid[m])
        #             sql = "insert into {0} (group_id,sub_group_id,key_node,rdsectid) values(%s,%s,%s,%s)"\
        #                 .format(CONTENT.alarm_subset)
        #             cr.execute(sql, (group_id, sub_group_id, key_node_id, sub_rdsectid[m]))
        #             conn.commit()
        #     print(chose_rdsectid, rdsectid_set)
        #     other_rdsectid = compare_list(chose_rdsectid, rdsectid_set)
        #     print(other_rdsectid)
        # else:
        #     # 如果没有拥堵子团，根据单点报警数量判断关键节点
        #     pass
        #     node_list = []
        #     alarm_num = []
        #     rdsectid = []
        #     for j in range(len(group_result)):
        #         if list(group_result[j].keys())[0] == group_id:
        #             rdsectid_set = list(i.values())[0][0]
        #             # print(rdsectid_set[0])
        #             all_node = list(group_result[j].values())[0]
        #             # print(group_result)
        #             for m in range(len(all_node)):
        #                 match_data = group_data[group_data['scats_id'] == all_node[m]]
        #                 if len(match_data) > 0:
        #                     alarm_num.append(match_data.iloc[0]['alarm_times'])
        #                     rdsectid.append(match_data.iloc[0]['alarm_times'])
        #                     node_list.append(all_node[m])
        #                     # print(node_list)
        #                 else:
        #                     pass
        #             max_num = max(alarm_num)
        #             index = alarm_num.index(max_num)
        #             key_node = node_list[index]
        #
        #             sub_group_id = 'subset_'+str(key_node)
        #             sql = "insert into {0} (group_id,sub_group_id,key_node,rdsectid) values(%s,%s,%s,%s)" \
        #                     .format(CONTENT.alarm_subset)
        #             for n in range(len(rdsectid_set)):
        #                 cr.execute(sql, (group_id, sub_group_id, key_node, rdsectid_set[n]))
        #                 conn.commit()
...
if __name__ == '__main__':
    # node_suggest()
    main('2018-03-05', '2018-03-15', '07:00:00', '09:00:00')

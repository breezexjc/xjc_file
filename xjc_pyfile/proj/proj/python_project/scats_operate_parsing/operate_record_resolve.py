...
def resolve_operate_data(data):
    ...
    aaa=pd.DataFrame(aa)
    print(df_resolve_data)
    print(aa)
    print(len(aa))
    #df_resolve_data.to_csv(r'F:\正则匹配\test.csv', index=0)  #本地操作的时候使用
    #aaa.to_csv(r'F:\正则匹配\shuju\23.csv', index=0)
    #i=0
    #wb=openpyxl.load_workbook('F:\正则匹配\人工操作记录 _样例解释.xlsx')
    #ws=wb.worksheets[0]
    #ws.insert_cols(12)
    #for index,rows in enumerate(ws.rows):
            ...
            #rows[i].value="record"
        #else:
            #print(resolve_data[i])
            #rows[i].value=str(resolve_data[i])
            #i+=1
    #wb.save('sad.xlsx')


    #df_resolve_data = pd.DataFrame(resolve_data, columns=['userid', 'region', 'siteid', 'oper_time', 'oper_code', 'record_resolve'])
    #print(df_resolve_data)
    #s = True
    #n = 0
    #while s:
    #   try:
    #        df_resolve_data.to_csv(r'F:\正则匹配\shuju\record%s.csv', index=0)
    #    except Exception as e:
    #        n += 1
    #        pass
    #    else:
...
def main():
    #f = xlrd.open_workbook(r'F:\正则匹配\人工操作记录 _样例解释.xlsx','rb')
    #sheet = f.sheet_by_name('人工操作记录样例数据')
    #cols=sheet.col_values(10)
    #operate_data=cols
    #operate_data = call_oracle_operate_data('2018-09-10 00:00:00')
